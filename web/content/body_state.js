...
export function organLines(who) {
  ...
  if (o.labia.swell >= 2) bits.push("陰唇腫脹、敏感。");
  if (o.clit.swell >= 2) bits.push("陰蒂又硬又脹。");
  return bits;
}
...
export function bodyPromptLines(who) {
  ...
  const tone = [];
  if (lib === "peak" || ar === "climax" || ar === "wantFill") {
    tone.push("語氣要更喘、更急、句子容易被快感打斷；可以主動求摸／求插，但仍用台詞演出，不要旁白。");
  } else if (lib === "high" || ar === "aroused") {
    tone.push("語氣帶點黏與散漫，心思容易飄到身體上；偶爾露骨但不失控。");
  } else if (lib === "mid" || ar === "slight") {
    tone.push("語氣比平常多一點曖昧／不好意思，身體感若有若無。");
  } else {
