...
| **M4** | 短 AI：出卡即時、兩拍、作廢 gen；**無整輪預產** | ✅ 收尾 | 書 2§12.2、persona_builder |
| **M5** | 產製 CG cache／占位 | 待做 | 書 2§4／12.3、README、comfy |
| **M6** | 遷移舊存檔、刪死聊天路徑 | 待做 | 書 2§13、書 6 |

**v1 不做：** 多節卡 `steps[]` 演出（書 2 §5.4）— 重開前先改規格。

每期結束跑 **書 2 §14 驗收清單** 對應項（含 §14.6 M4）。
...
**若接 M5／M6：** 先讀書 2 §12.3／§13，不要再重做 M4 預產。
