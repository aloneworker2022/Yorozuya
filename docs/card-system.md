...
> **狀態：待實作規格（design lock）**  
> 這份文件描述的是**已鎖定、準備實作**的玩法，不是目前 `app.js` 已上線的行為。  
> 實作時以本文為準；與 `plan-v5.md` 衝突處，**以本文覆寫「聊天／淫紋聊天／舊約會流程」相關段落**。  
> 企劃書索引：`plan-v5.md` **§13A**。  
> **卡牌／場地資料：[`web/content/cards.json`](../web/content/cards.json)**（完整可載入，勿再手搓最小假卡）。  
...
若 普通卡：
  ...
  感情骰 → 演出（單拍或多節）→ shatter 若需 → 扣次數 → 補牌
  │
  ▼
多節卡：節內「停手」→ 立刻結束卡、已使用、碎
  │
  ▼
...
- 核心：規則、骰、碎、鍊、N、氣泡機率。  
- 內容模組：`sceneStart`、steps 文案、少女本體卡文案、氣泡句、CG prompt。  
...
```js
buildCardPlayPrompt({
  girl, player, relationship, stageAxes,
  card, step, sessionFlags, chain,
  worldLoreSnippet,
})
...
1. **資料**：`cards.json` 最少 10 話術 + 5 碎卡 + 2 開門 + 1 不可走；inventory／shop 刷新。  
2. **無 AI 打牌**：純文字 sceneStart + 罐頭反應 + 骰 + 碎 + N + 輪末留下。  
3. **氣泡 15%** 接上三個委託 API。  
4. **創角 10 選 1**。  
5. **鍊** 完整。  
6. **約會** 電話＋場地 3 卡。  
7. **短 AI 反應** 接 gener 佇列。  
8. **生圖預產** cache。  
9. 舊聊天入口隱藏／刪。  
